...
import { OptOutSchemaColumnTool } from './opt-out-schema-column-tool';

describe('OptOutSchemaColumnTool', () => {
  ...
  it('successfully sets a column to opt-out by default', async () => {
    mocks.setResponse({
      opt_out_schema_column: { id: '42', name: 'my_schema', description: null, parent_id: null, revision: 2 },
    });
    const tool = new OptOutSchemaColumnTool(mocks.mockApiClient);

    const result = await tool.execute({ schemaId: '42', columnId: 'col-1' });

    expect((result.content as { message: string }).message).toContain('opt-out');
  });

  it('passes versionOverride dev to API request', async () => {
    mocks.setResponse({
      opt_out_schema_column: { id: '1', name: 'test', description: null, parent_id: null, revision: 2 },
    });
    const tool = new OptOutSchemaColumnTool(mocks.mockApiClient);
  ...
  it('accepts schemaName instead of schemaId', async () => {
    mocks.setResponse({
      opt_out_schema_column: { id: '5', name: 'my_schema', description: null, parent_id: null, revision: 3 },
    });
    const tool = new OptOutSchemaColumnTool(mocks.mockApiClient);
  ...
  it('has correct tool properties', () => {
    ...
    expect(tool.name).toBe('opt_out_schema_column');
