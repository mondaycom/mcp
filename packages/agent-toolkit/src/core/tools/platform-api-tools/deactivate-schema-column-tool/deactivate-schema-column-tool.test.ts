...
import { DeactivateSchemaColumnTool } from './deactivate-schema-column-tool';

describe('DeactivateSchemaColumnTool', () => {
  ...
  it('successfully deactivates a schema column', async () => {
    mocks.setResponse({
      deactivate_schema_column: { id: '42', name: 'my_schema', description: null, parent_id: null, revision: 2 },
    });
    const tool = new DeactivateSchemaColumnTool(mocks.mockApiClient);
    ...
    expect((result.content as { message: string }).message).toContain('deactivated');
  });

  it('passes versionOverride dev to API request', async () => {
    mocks.setResponse({
      deactivate_schema_column: { id: '1', name: 'test', description: null, parent_id: null, revision: 2 },
    });
    const tool = new DeactivateSchemaColumnTool(mocks.mockApiClient);
  ...
  it('accepts schemaName instead of schemaId', async () => {
    mocks.setResponse({
      deactivate_schema_column: { id: '5', name: 'my_schema', description: null, parent_id: null, revision: 3 },
    });
    const tool = new DeactivateSchemaColumnTool(mocks.mockApiClient);
  ...
  it('has correct tool properties', () => {
    ...
    expect(tool.name).toBe('deactivate_schema_column');
