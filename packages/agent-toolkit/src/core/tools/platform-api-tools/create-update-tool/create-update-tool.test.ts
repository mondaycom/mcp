...
describe('Create Update Tool', () => {
  ...
  it('Successfully creates an update without mentions', async () => {
    ...
    expect(result.content).toBe(JSON.stringify({ message: 'Update 123456789 created on item 456', update_id: '123456789', item_id: 456 }));
  ...
  it('Successfully creates an update with mentions', async () => {
    ...
    expect(result.content).toBe(JSON.stringify({ message: 'Update 123456789 created on item 789', update_id: '123456789', item_id: 789 }));
    expect(mocks.getMockRequest()).toHaveBeenCalledWith(expect.stringContaining('mutation createUpdate'), {
      itemId: '789',
      body: 'Hey, check this out!',
      mentionsList: [
