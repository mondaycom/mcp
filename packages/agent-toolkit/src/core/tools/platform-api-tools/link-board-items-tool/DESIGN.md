...
| `searchTerm`      | string               | no                   | Full-text search term resolved server-side via the smart search API (searches across name, text columns, notes, etc.). Falls back to a `name ContainsText` filter on timeout or if smart search is not enabled. Intersected with `itemIds` if both are provided. |
...
- `source.searchTerm` / `target.searchTerm` resolve via the smart search API before the main fetch — results become the effective `itemIds` (intersected with any caller-provided `itemIds`). Falls back to a `name ContainsText` filter on timeout or if smart search is not enabled for the account.
