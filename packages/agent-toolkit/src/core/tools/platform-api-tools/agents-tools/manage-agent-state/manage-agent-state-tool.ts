...
- deactivate: Transitions the agent to INACTIVE.
- run: Manually enqueues a run of the agent (fire-and-forget). Returns a trigger_uuid for future correlation. There is no run-status query yet — treat "Agent run enqueued" as the only success signal.
    ...
    if (input.action === 'activate') {
      ...
      try {
        const deactivateRes = await this.mondayApi.request<DeactivateAgentMutation>(
          ...
          {
            id: input.agent_id,
            inactive_reason: InactiveReason.DeactivatedByUser,
