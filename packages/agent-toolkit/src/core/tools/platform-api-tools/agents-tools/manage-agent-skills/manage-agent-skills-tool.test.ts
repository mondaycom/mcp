...
describe('ManageAgentSkillsTool', () => {
  ...
  describe('action: add', () => {
    ...
    it('should reject add without skill_id', async () => {
      const result = await callToolByNameRawAsync('manage_agent_skills', { action: 'add', agent_id: '7' });
  ...
  describe('action: remove', () => {
    ...
    it('should reject remove without agent_id', async () => {
      const result = await callToolByNameRawAsync('manage_agent_skills', { action: 'remove', skill_id: 'skill-abc-123' });

      expect(result.content[0].text).toContain('agent_id is required');
    });

    it('should reject remove without skill_id', async () => {
      const result = await callToolByNameRawAsync('manage_agent_skills', { action: 'remove', agent_id: '7' });
