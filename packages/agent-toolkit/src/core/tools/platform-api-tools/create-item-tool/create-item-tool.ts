...
export class CreateItemTool extends BaseMondayApiTool<CreateItemToolInput> {
  ...
  getDescription(): string {
    return (
      'Create a new item with provided values, create a subitem under a parent item, or duplicate an existing item and update it with new values. Use parentItemId when creating a subitem under an existing item. Use duplicateFromItemId when copying an existing item with modifications. To create multiple items in one call, use create_items instead of calling this tool in a loop.' +
