...
- `create_column` / `update_column` — call `get_column_type_info` before populating `columnSettings` (previously only stated on the field description). `update_column` keeps the existing `get_board_schema` revision precondition
...
- `get_graphql_schema` / `get_type_details` — call both before crafting an operation for `all_monday_api`, `all_api_read`, or `all_api_write`
