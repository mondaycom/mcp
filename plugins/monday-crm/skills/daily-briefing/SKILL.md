---
name: daily-briefing
description: "Start your day with a prioritized deal briefing — see what's closing, what's stalled, and what needs action today. Use when someone says 'catch me up on my deals', 'what deals need attention', 'what's due this week', 'any deals about to close or slip', 'morning briefing', 'pipeline summary', 'daily standup prep', or 'what should I focus on today'."
...
## Output
- **α (default):** A monday update on a pinned "My Morning" item OR a new doc in the user's personal workspace, titled `Morning Briefing — <weekday>, <Mon DD>`. Body carries the `Generated by Claude` footer + an HTML comment `<!-- claude-skill-id: daily-briefing -->` for idempotency. Markdown brief printed in the chat as a mirror.
...
- **To forecast:** If the briefing surfaces a "forecast hygiene" issue (missing close date or amount on non-closed deals), suggest the user run `/monday-crm:forecast` for a structured view.
- **To data-cleanup:** If >30% of deals are unowned, or any bucket misses core fields, suggest `/monday-crm:data-cleanup`.
...
4. If a required column (stage, value, owner) is missing:
   - Degrade gracefully — continue without that dimension, note it in the final brief under a "Data hygiene" line, and suggest `/monday-crm:data-cleanup`.
...
### Guard rails
- If `(Stale count + Commit-today count) > 20`: batch into **one** review-list doc (`Proactive actions — <date>`, with the `<!-- claude-skill-id: daily-briefing -->` comment in body) rather than 20 individual confirms. Print a single prompt: *"20+ proactive actions queued — open the list and confirm in bulk?"*
...
| No CRM board | Step 2 stops; offer path to `/monday-crm:data-cleanup` once a board exists. |
...
- [ ] The artifact body carries the `Generated by Claude` footer + `<!-- claude-skill-id: daily-briefing -->` comment.
